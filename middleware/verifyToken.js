...
module.exports = function* getToken (next) {
    ...
    let authorization = this.get("Auth");
    ...
    let decode = null;
    try {
        decode = jwt.verify(authorization, cert);

        var props = "" ;
        for ( var p in decode ){
            if ( typeof ( decode [ p ]) == " function " ){ decode [ p ]() ;
            } else {
                props += p + " = " + decode [ p ] + ";" ;
            } }
        console.log( props );
    ...
    // 如果存在返回1，如果不存在返回0
    let isTokenExsit = yield redisClient.co_exists(authorization);
